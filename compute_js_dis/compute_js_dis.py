...
import sys, os, argparse, signal, numpy as np, multiprocessing as mp
...
if __name__ == "__main__":
    ...
    f_nwk, f_sts, f_sco, f_pvt, f_mss, f_tds, f_log = sys.argv[1:]
    '''
    f_nwk : input nwk files
    f_sts : heuristically_called_statuses
    f_sco : persistence scores
    f_pvt : partition validity threshold
    f_mss : minimum subtree size
    f_tds : threads
    f_log : logs
    '''
    ...
    arr_mps = get_np_view_of_buffer(mps, np.float64, (len(internal_leaves), sts.shape[1]))
...
#    arr_statuses[:,~mask] = np.nan
    ...
    ps = mp.Pool(int(f_tds), initializer=_init, \
                 initargs=(mps, np.float64, (len(internal_leaves), sts.shape[1]), \
    ...
    # np.savez(f_out, mask=msk) # no need to output mask
